...
def parse_arguments():
    ...
    print("")
    parser.add_argument('-p', help='specify the printer type (optional, default=DMP2000)', dest='printer', default='DMP2000', choices=['DMP1','DMP2000'], required=False)
    parser.add_argument('-o', help='output filename, (required, default=printerpage.png)', dest='outfile', required=True, action='store', default='printerpage.png')
    parser.add_argument('-i', help='input filename, (required, default=printer.dat)', dest='infile', required=True, action='store',default='printer.dat')
    parser.add_argument('-v','--verbose', help='Display some information about what this is doing', dest='verbose', action='store_true')
                        ...
                        match ord(ascii_text[printout]):
                            ...
                            case 0x1b:
                                ...
                                match next:
                                    ...
                                    case 0x28:
                                        print ("Location:",hex(printout)," Unimplemented code 0x1b,0x28")
                                        skipcount=2
                                    case 0x33:
                                    ...
                                    case 0x42:
                                        if debug:
                                            print ("Processing 0x1b 0x42 - Next bytes:",hex(ord(ascii_text[printout+1])),hex(ord(ascii_text[printout+2])),hex(ord(ascii_text[printout+3])))
                                        skipcount=2
                                        tabcnt=1
                                        while ord(ascii_text[printout+tabcnt])!=0:
                                            tabcnt=tabcnt+1
                                            skipcount=skipcount+1
                                            print ("Found tab:",ord(ascii_text[printout+tabcnt]),"(",hex(ord(ascii_text[printout+tabcnt])),")")
                                    case 0x4a:
                                        #1B 4A n 	27 74 n 	ESC "J" n 	Feed 	Perform One-shot n/216 inch line feed (n=0..255)
                                        if debug:
                                            print ("Processing 0x1b 0x42 one shot feed to",int(216/(next)*2))
                                        skipcount=3
                                        h=h+int(216/(next)*2)
                                    case 0x4b:
                                    ...
                                    case 0x4c:
                                        ...
                                        skipcount=4
                                        w=paperwidth(960) #If we're using 120dpi, we need to double the default "paper" we setup (was 480)
                                        #Some 7 bit math to see how much we should ignore before we process more data
                                        gfxdata=(ord(ascii_text[printout+3])*256)+ord(ascii_text[printout+2])
                                        if debug:
                                    ...
                                    case 0x5a:
                                        ...
                                        skipcount=4
                                        w=paperwidth(1920) #If we're using 240dpi, we need to quadruple the default "paper" we setup (was 480)
                                        #Some 7 bit math to see how much we should ignore before we process more data
                                        gfxdata=(ord(ascii_text[printout+3])*256)+ord(ascii_text[printout+2])
                                        if debug:
                                            print ("Loc:",hex(printout),"has data ",gfxdata,"bytes  from ",hex(ord(ascii_text[printout+2]))," and", hex(ord(ascii_text[printout+3])))
                                    ...
                                    case 0x78:
                                        skipcount=3
                                        print ("Location:",hex(printout)," Unimplemented 0x1b function found: 0x1b,",hex(ord(ascii_text[printout+1])))
                                    case _:
                                        print ("Location:",hex(printout)," Unimplemented 0x1b function found: 0x1b,",hex(ord(ascii_text[printout+1])))
                                        skipcount=2
                            case 0x1c:
                            ...
                            case _:
                                ...
                                if (value>31):
                                    ...
                                    print ("Location:",hex(printout)," Unimplemented function found: ",hex(ord(ascii_text[printout])))
