...
def validate_arguments(argslist):
    ...
    for param in argslist.ROMfile:
        tmp=param.upper()
        if tmp.endswith('.COM'):
            err("Error! Destination ROM filename ends with '.COM'!\n Did you forget to include the ROM filename?")
        else:
            romfile=param
            if argslist.debug:
    ...
    for param in argslist.id:
        if len(param)>16:
            err(f'Error! ROM id is too long - maximum size is 16 characters. You used {len(param)} characters')
    ...
    for param in argslist.romversion:
        if len(param)>16:
            err(f'Error! ROM version is too long - maximum size is 16 characters. You used {len(param)} characters')
        # elif not param.startswith("G"):
        #     print("The ROM id must start with 'G' otherwise the CP/M rom will not recognize it!")
        #     quit(1)
        else:
    ...
    if argslist.name==None:
        ...
        for param in argslist.name:
            if len(param)>22:
                err(f'Error! ROM name is too long- maximum size is 22 characters. You used {len(param)} characters')
            else:
...
''')
    ...
    group1=parser.add_argument_group('Building', 'These options relate to building a ROM')
    group2=parser.add_argument_group('Extracting', 'These options relate to extracting files from a ROM')
    group0.add_argument('ROMfile', type=str, default='',metavar='ROM_filename',
                         nargs=1, help='This is the name of the ROM file that you are creating.')

    group1.add_argument('COMfiles', type=str, default='',metavar='.COM_file',
                        nargs='*', help='This must be a CP/M program.')

    group1.add_argument("-i", "--id", nargs=1, default='',metavar='"ROM ID String"',
        help="ROM Identification. It must start with G, and be no more than 16 characters. The default is "+chr(34)+rom_id+chr(34))

    group1.add_argument('-n', '--name', nargs=1,metavar='"ROM Description"',
         help='This is the name that will be shown in commands such as |OHELP or romcat. It must be no more than 32 characters.')

    group1.add_argument('-v', '--romversion', nargs=1, default=['V1.00'],metavar='"ROM Version string"',
         help='This is a string that will dictate the version of the rom.')

    group2.add_argument('-e', '--extract', nargs='*' ,metavar='filename, or blank for all files',
...
def add_file(filename):
    ...
    fn_padded=fn_padded.upper()[0:9] #because modern OSs can have long files, CP/M wont, so chop it down.
...
code_array = [0x32,0xbe,0x3f,0xe5,0xdd,0xe1,0x2a,0xfe,0xfb,0x11,0x34,0x0,0x19,0x11,
    0x8c,0x3f,0x73,0x23,0x72,0x2a,0xfe,0xfb,0x11,0x10,0x0,0xe,0x3b,0xe9,0xdd,0x6e,
    0x0,0xdd,0x66,0x1,0xdd,0x4e,0x2,0xdd,0x46,0x3,0x11,0x0,0x1,0xcd,0xbf,0x3f,0x12,
    0x13,0x23,0xb,0x79,0xb0,0x20,0xf5,0x1e,0xff,0xe,0x2d,0xcd,0x5,0x0,0x3a,0xaf,0xfb,
    0x5f,0xe,0xe,0xcd,0x5,0x0,0x1e,0x0,0xe,0x2d,0xcd,0x5,0x0,0xc9,0x0,0xc5,0x3a,0xbe,
    0x3f,0x4f,0xcd,0xc9,0x3f,0xc1,0xc9,0xf3,0xc5,0xd9,0xcb,0x99,0xed,0x49,0xd9,0x6,
    0xdf,0xed,0x49,0x7e,0xd9,0xcb,0xd9,0xed,0x49,0xd9,0xfb,0xc1,0xc9,0xed,0x49,0xd9,
    0x6,0xdf,0xed,0x49,0x7e,0xd9,0xcb,0xd9,0xed,0x49,0xd9,0xfb,0xc1,0xc9]
